...
CollisionResolver physicsWorld;

void setup() {
  size(480, 480);
  physicsWorld = new CollisionResolver();
}
...
void mousePressed() {
  physicsWorld.mPressed();
}


class CollisionResolver {

  SimpleBox box1 = new SimpleBox(200, 100, 80, 0.3); // x, y, size, bounce
  ...
  float penetration; // depth into a collision
  PVector normal; // of the collision
  float percent = 0.2; // mass percentage to move each object
  ...
  boolean circlesColliding(SimpleCircle a, SimpleCircle b) {
    PVector distVect = PVector.sub(b.pos, a.pos);
// find the distance between the centers
    float distance = distVect.mag();
// add together the radii
    float sumRadii = a.r + b.r;
// if the distance is less than the sum, they are colliding 
  ...
  boolean boxesColliding(SimpleBox a, SimpleBox b) {
    PVector distVect = PVector.sub(b.pos, a.pos);
    float x_overlap = a.halfW + b.halfW - abs(distVect.x);

    // use the axis of least penetration to resolve collision
    if (x_overlap > 0) {
      float y_overlap = a.halfW + b.halfW - abs(distVect.y);
  ...
  boolean boxAndCircleColliding(SimpleBox a, SimpleCircle b) {
    ...
    PVector closest = new PVector(distVect.x, distVect.y);
    ...
    if (distVect.x == closest.x && distVect.y == closest.y) {
      inside = true;
      if (abs(distVect.x) > abs(distVect.y)) {
        if (closest.x > 0) {
          closest.x = a.halfW;
        } 
        else {
    ...
    normal = PVector.sub(distVect, closest);
    float dist_squared = normal.x*normal.x + normal.y*normal.y;
    float r = b.r;

    if (dist_squared > r*r && !inside) {
      return false;
    }
  ...
  void resolveCollision(PVector velA, PVector velB, float restitutionA, float restitutionB, float invMassA, float invMassB) {
    ...
    PVector relavtiveVelocity = PVector.sub(velB, velA);

    float velAlongNormal = relavtiveVelocity.dot(normal);
    if (velAlongNormal > 0) { // velocities are separating
      return;
    }

    // use Newton Coefficient of Restitution
    float restitution = min(restitutionA, restitutionB); 

    float scalar = (1+restitution) * -velAlongNormal;
    scalar /= invMassA + invMassB;

    PVector impulse = PVector.mult(normal, scalar);
...
class SimpleBox {
  ...
  SimpleBox(float xCenter, float yCenter, float radius, float restitution) {
